...
import type { PesquisaBoxProps } from "@/types/types.ts";
import { useNavigate } from "react-router-dom";
import ModalDesistencia from "./ModalDesistencia"; // Certifique-se que o caminho está certo
...
export default function PesquisaBox({ consumo }: PesquisaBoxProps) {
    ...
    const [showExitConfirmation, setShowExitConfirmation] = useState(false); // Estado do Modal de Desistência
    ...
    const handleInteraction = (responseType: 'negative' | 'positive') => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                setMessages(prev => [...prev, { id: Date.now(), type: 'system', content: <p className="text-sm font-bold">De 0 a 5, como você avaliaria esse serviço?</p> }]);
                setStep(3);
            }, 1000);
        }, 1000);
    };
    ...
    const handleRating = (rating: number) => {
        ...
        let followUpMessage = "";
        switch (rating) {
            case 0: case 1: case 2: followUpMessage = "que pena! 😟 o que aconteceu?"; break;
            case 3: followUpMessage = "hurrm... 😬 o que poderia ser diferente?"; break;
            case 4: followUpMessage = "que bom! 😊 e o que dá pra melhorar?"; break;
            case 5: followUpMessage = "que massa! 🥳 do que você mais gostou?"; break;
            default: followUpMessage = "Obrigado pela nota! Gostaria de deixar um comentário?";
        }
        setTimeout(() => {
            setMessages(prev => [...prev, { id: Date.now(), type: 'system', content: <p className="text-sm">{followUpMessage}</p> }]);
            setStep(4);
        }, 1200);
    };
    ...
    const handleSendText = async () => {
        ...
        try {
            ...
            if (!res.ok) throw new Error("Falha ao enviar pesquisa");
            ...
            await new Promise(r => setTimeout(r, 1000));
            setMessages(prev => [...prev, { id: Date.now(), type: 'system', content: <p className="text-sm font-bold">Obrigado pelo feedback, {nomeCapitalizado}!</p> }]);

            await new Promise(r => setTimeout(r, 1200));
            setMessages(prev => [...prev, { id: Date.now() + 1, type: 'system', content: <p className="text-sm">Sua opinião é essencial para continuarmos melhorando! 🧩</p> }]);
            setStep(5);
            setTimeout(() => { setIsVisible(false); }, 2500);

        } catch (error) {
            console.error("Erro no envio:", error);
            setMessages(prev => [...prev, { id: Date.now(), type: 'system', content: <p className="text-sm text-red-300">Ops! Tivemos um problema. Tente mais tarde.</p> }]);
            setStep(4);
        }
    };
    ...
    // Função para tratar o clique fora (ativa o modal de desistência)
        ...
        <AnimatePresence>
            {isVisible && (
                <motion.section
                    ...
                    animate="visible"
                    exit="exit"
                    onClick={handleBackdropClick} // <--- ADICIONADO: Clique fora abre o modal
                >
                    <style>{`
                    ...
                    <motion.section
                        ...
                        onClick={(e) => e.stopPropagation()} // <--- MANTIDO: Impede que clicar DENTRO abra o modal
                    >
                        {/* CABEÇALHO */}
                        ...
                        <section className="mt-4 bg-[#1a1a40] rounded-full p-1 flex items-center justify-between h-16 border border-blue-900/30 shadow-lg shrink-0 overflow-hidden relative">
                            <AnimatePresence mode="wait">
                                ...
                                {step === 1 && (
                                    <motion.div key="step1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex w-full h-full">
                                ...
                                {step === 3 && (
                                    <motion.div key="step3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex w-full h-full justify-between items-center px-4">
                                ...
                                {step === 4 && (
                                    <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex w-full h-full items-center px-2 gap-2">
                                        <input
                                            type="text"
                                            value={inputValue}
                                            ...
                                            autoFocus
                                        />
                                        <button onClick={handleSendText} className="bg-[#0099ff] hover:bg-[#007acc] text-white p-2 rounded-full transition-colors flex items-center justify-center w-10 h-10 shrink-0">
                    ...
                    {/* RENDERIZAÇÃO CONDICIONAL DO MODAL DE DESISTÊNCIA */}
                    <AnimatePresence>
                        {showExitConfirmation && (
                            <ModalDesistencia onCancel={() => setShowExitConfirmation(false)} />
                        )}
                    </AnimatePresence>
